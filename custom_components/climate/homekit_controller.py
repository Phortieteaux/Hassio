...
class HomeKitClimateDevice(HomeKitEntity, ClimateDevice):
    ...
    def update_characteristics(self, characteristics):
        ...
        for characteristic in characteristics:
            ...
            if ctype == ctypes.get_uuid(ctypes.HEATING_COOLING_CURRENT):
                self._current_mode = MODE_HOMEKIT_TO_HASS.get(
                    characteristic['value'])
            if ctype == ctypes.get_uuid(ctypes.HEATING_COOLING_TARGET):
                self._chars['target_mode'] = characteristic['iid']
                self._features |= SUPPORT_OPERATION_MODE
                self._target_mode = MODE_HOMEKIT_TO_HASS.get(
                    characteristic['value'])
                #self._valid_modes = [MODE_HOMEKIT_TO_HASS.get(
                #    mode) for mode in characteristic['valid-values' ]]
            elif ctype == ctypes.get_uuid(ctypes.TEMPERATURE_CURRENT):
                self._current_temp = characteristic['value']
            elif ctype == ctypes.get_uuid(ctypes.TEMPERATURE_TARGET):
    ...
    def state(self):
        ...
        # report a new state.
        if self._target_mode == STATE_OFF:
            return STATE_OFF

        if self._current_mode == STATE_OFF and self._target_mode != STATE_OFF:
            return STATE_IDLE
